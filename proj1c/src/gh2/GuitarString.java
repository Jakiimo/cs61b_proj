...
public class GuitarString {
    ...
    public void pluck() {
        ...
        int capacity_temp = buffer.size();
        while (!buffer.isEmpty()) {
            buffer.removeFirst();
        }
        for (int i = 0; i < capacity_temp; i++) {
    ...
    public void tic() {
        ...
        double newSample = DECAY * (first + second) / 2.0;//移出去的队首+新队首
        buffer.addLast(newSample);//新采样入队
    }

    /* Return the double at the front of the buffer. */
    public double sample() {
        // TODO: Return the correct thing.
        return (double) buffer.get(-1);
    }
}
